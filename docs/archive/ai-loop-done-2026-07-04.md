...
### 2026-07-04 | v1.0 | RAGas 评测闭环和法律知识库 0.88+ 质量验收归档
- summary: 将 RAGas 知识库级自动评测归档为 `v1.0`：新增评测运行、逐题样本、黄金评测集和 baseline 数据模型；新增 `/api/v1/evaluations` 与 `/api/v1/evaluations/testsets` API；Celery 任务 `evaluations.run` 复用知识库绑定 QA / Embedding 模型调用 Quick Q&A 并计算五项 0-1 指标；前端新增 `/#/evaluations` 页面展示总分、指标分布、baseline/current 对比、逐题答案、参考答案、sources 和诊断；大批量评测默认使用 `semantic_proxy` guard 并保存 `evaluator_config`；法律知识库通过结构化法律 metadata、legal exact lookup、legal boost、source 命中诊断和更严格法律回答 prompt，将 50 题黄金集复测总分稳定提升到 0.88+。
- files: `README.md`, `CHANGELOG.md`, `docs/v1.0.md`, `docs/ai-loop/requirements.md`, `docs/ai-loop/done.md`, `app/api/v1/evaluations.py`, `app/db/models.py`, `app/schemas/evaluation.py`, `app/services/evaluation.py`, `app/rag/ragas_adapter.py`, `app/rag/legal_structure.py`, `app/services/legal_metadata.py`, `app/services/knowledge_search.py`, `app/rag/prompt.py`, `app/workers/tasks.py`, `scripts/evaluation_ab.py`, `scripts/backfill_legal_metadata.py`, `frontend/src/views/EvaluationsView.vue`, `frontend/src/stores/evaluations.ts`, `frontend/src/types/api.ts`, `tests/test_v10_ragas_evaluations.py`, `tests/test_frontend_v10_evaluations.py`
- verification: `python -m pytest -q` -> 253 passed；`python -m pytest tests/test_v10_ragas_evaluations.py tests/test_frontend_v10_evaluations.py -q` -> 11 passed；`ruff check .` -> All checks passed；`python -m compileall app tests` -> exit 0；`npm --prefix frontend run build` -> passed with existing Vite large chunk warning；法律知识库黄金集 `26f4a44b-91f4-4479-a6a2-a42e92218e5a` 基线 run `0d160b0c-31ae-490a-a06e-2bdaaa087a8e` 为 0.5228，最终 run `d675c9a7-4abc-4c8a-8e25-6e1d576fbc85` 为 0.8822，复测 run `0f650beb-ba66-471e-814b-48b17e5c9cb9` 为 0.8819，波动 0.0003；`.runtime-logs/ab-legal-exact-v3.json` 显示 `recall_at_10=0.98`、`precision_at_5=0.98`、`miss_count=1`、`failed=0`。
- follow_ups: v1.0 之后的评测增强集中在人工基准集导入、评测取消按钮、成本预算、MRR/nDCG、定时回归、Langfuse/Phoenix 观测集成和更多领域知识库的稳定性复测。
